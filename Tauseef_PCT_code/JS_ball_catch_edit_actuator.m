...
%use fill methods: 'center', 'clip', 'previous', 'next', 'nearest', 'linear', 'spline', 'pchip', 'makima'
angley1= filloutliers(angley1,'next','movmedian',10);
angley3= filloutliers(angley3,'next','movmean',10);
    %be sure to update in while loop code as well
    if y1_OG(n)>y2_OG(n)
        angley2=0.5*pi-angley1; %internal angles of a right-angle-triangle
    else %y2>y1
        angley2= -(0.5*pi-(pi-angley1)); %internal angles of a right-angle-triangle
    end
%figure;plot(angley1);hold on;plot(angley2);plot(angley3);legend('angle1','angle2','angley3')

%need to make bh in the correct Frame of Reference (not x, but box ways)
if y1(1)>y2(1)
    bh(1)=len/2*angley1(1);
else
    bh(1)=len/2*(angley1(1)-pi/2);
end
bh_OG(1)=bh(1);
...
v(1)=bh(1)-(5-y2(1)*sin(angley3(1))); vp=v(1);
...
while (d > 0) 
    for t=2:length(y1)
            ...
            lvel(t) = lvel(t-1) - (lgain * (ref2-delay(3,1))-lvel(t-1))/lslow; 
